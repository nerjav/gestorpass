<div class="form-group row align-items-center" :class="{'has-danger': errors.has('admin_users'), 'has-success': fields.admin_users && fields.admin_users.valid }">
    <label for="admin_users" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.verification.columns.admin_users') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.admin_users" v-validate="'required|integer'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('admin_users'), 'form-control-success': fields.admin_users && fields.admin_users.valid}" id="admin_users" name="admin_users" placeholder="{{ trans('admin.verification.columns.admin_users') }}">
        <div v-if="errors.has('admin_users')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('admin_users') }}</div>
    </div>
</div>
...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('password'), 'has-success': fields.password && fields.password.valid }">
    ...
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="password" v-model="form.password" v-validate="'min:7'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('password'), 'form-control-success': fields.password && fields.password.valid}" id="password" name="password" placeholder="{{ trans('admin.verification.columns.password') }}" ref="password">
